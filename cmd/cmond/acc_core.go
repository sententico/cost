...
func ec2awsClean(m *model, deep bool) {
	...
	for id, inst := range detail.Inst {
		if x := detail.Current - inst.Last; inst.Last-inst.Since < 3600 && x > 3600*3 || x > 3600*72 {
...
func ebsawsClean(m *model, deep bool) {
	...
	for id, vol := range detail.Vol {
		if x := detail.Current - vol.Last; vol.Last-vol.Since < 3600 && x > 3600*3 || x > 3600*72 {
...
func rdsawsClean(m *model, deep bool) {
	...
	for id, db := range detail.DB {
		if x := detail.Current - db.Last; db.Last-db.Since < 3600 && x > 3600*3 || x > 3600*72 {
			delete(detail.DB, id)
		}
	}
	exp := sum.Current - 24*100
